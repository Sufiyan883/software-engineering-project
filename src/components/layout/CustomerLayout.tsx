...
import { 
  Home, BarChart3, Receipt, Lightbulb, User, MessageSquare, LogOut 
} from 'lucide-react';
...
const CustomerLayout: React.FC<CustomerLayoutProps> = ({ children }) => {
  ...
  const navItems = [
    ...
    { name: 'Usage', path: '/customer?tab=usage', icon: <BarChart3 size={20} /> },
    { name: 'Bills', path: '/billing', icon: <Receipt size={20} /> },
    { name: 'Energy Tips', path: '/tips', icon: <Lightbulb size={20} /> },
    { name: 'Profile', path: '/customer?tab=profile', icon: <User size={20} /> },
    { name: 'Support', path: '/contact', icon: <MessageSquare size={20} /> },
  ];
              ...
              <li key={item.name}>
                <Link
                  ...
                  className={`flex items-center space-x-3 p-2 rounded-lg transition-colors ${
                    ...
                    (location.pathname === '/customer' && item.path.startsWith('/customer?tab='))
